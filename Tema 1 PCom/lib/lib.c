...
{
	ssize_t ret;
	ret = read(interfaces[intidx], frame_data, MAX_PACKET_LENGTH);
	return ret;
}
...
int socket_receive_message(int sockfd, char *frame_data, size_t *len)
{
	 ...
	 * interface, eg 1500 bytes
	 * */
	int ret = read(sockfd, frame_data, MAX_PACKET_LENGTH);
	DIE(ret < 0, "read");
	*len = ret;
	return 0;
}
...
int recv_from_any_link(char *frame_data, size_t *length) {
	...
	while (1) {
		...
		res = select(interfaces[NUM_INTERFACES - 1] + 1, &set, NULL, NULL, NULL);
		DIE(res == -1, "select");
...
int read_rtable(const char *path, struct nod_arbore *rtable)
{
    ...
    {
        struct route_table_entry *intrare_noua = malloc(sizeof(struct route_table_entry));
        ...
        while (linie != NULL)
        {
            ...
            case 2:
            case 3:
                *(((unsigned char *) &intrare_noua->prefix) + contor % 4) = (unsigned char)atoi(linie);
                break;
            ...
            case 6:
            case 7:
                *(((unsigned char *) &intrare_noua->dest_urm) + contor % 4) = atoi(linie);
                break;
            ...
            case 10:
            case 11:
                *(((unsigned char *) &intrare_noua->mask) + contor % 4) = atoi(linie);
                break;

            case 12:
                intrare_noua->interface = atoi(linie);
                break;
        ...
        adauga_nod(intrare_noua, rtable);
